...
function extractLines(raw){
  if (raw?.lyrics?.lines) return raw.lyrics.lines.map(x => String(x ?? '')).filter(Boolean)
  if (Array.isArray(raw?.lyrics)) return raw.lyrics.map(l => l?.text ?? '').filter(Boolean)
  return []
}
...
async function deeplTranslateLinesKoToJa(lines){
  ...
  for (let i=0;i<lines.length;i+=batchSize){
    ...
    const need = []
    cached.forEach((v, idx) => { if (v == null) need.push({ idx, text: batch[idx] }) })
    if (need.length){
      ...
      if (!r.ok){
        need.forEach(({idx}) => { cached[idx] = batch[idx] })
      }else{
    ...
    cached.forEach((v, idx) => { out[i+idx] = v ?? batch[idx] })
  }
  return out
}
